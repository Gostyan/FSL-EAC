#!/usr/bin/env python3
"""
SSAMBA Transductive Fine-Tuning - 2048D Multi-Layer Feature Version

特点:
- 提取多层特征 (Layer 8, 16, 24): 3×768 = 2304D
- 投影到2048D (匹配PANNS维度)
- 其他配置与768D版本完全一致
...
class SSAMBAEncoder(nn.Module):
    """SSAMBA 编码器 - 2048D多层特征版本"""
    
    def __init__(self, pretrained_path, input_tdim=1024, freeze_early_layers=20):
        super().__init__()
        self.mamba_dim = 768  # 单层维度
        self.embed_dim = 2048  # 最终输出维度
        self.input_tdim = input_tdim
        # self.extract_layers = [7, 15, 23]  # 提取layer 8, 16, 24 (0-indexed)
        self.extract_layers = [11, 16, 23]  # Layer 12, 17, 24
    ...
    def _load_pretrained_model(self, pretrained_path):
        ...
        # 多层特征投影
        concat_dim = len(self.extract_layers) * self.mamba_dim  # 3 * 768 = 2304
        self.feature_projection = nn.Linear(concat_dim, self.embed_dim)
        nn.init.xavier_uniform_(self.feature_projection.weight)
        nn.init.zeros_(self.feature_projection.bias)
        
        self.norm = nn.LayerNorm(self.mamba_dim)  # 对每层norm
        print(f"[SSAMBAEncoder] Multi-layer features: layers {[l+1 for l in self.extract_layers]}")
        print(f"[SSAMBAEncoder] Concat: {concat_dim}D → Projection → {self.embed_dim}D")
        ...
        for i, layer in enumerate(self.v.layers):
            ...
            if i in self.extract_layers:
                ...
                if residual is not None:
                    layer_output = residual + x
                else:
                    layer_output = x
                cls_feat = self.norm(layer_output[:, 0])  # (B, 768)
                intermediate_features.append(cls_feat)
        
        # 拼接多层特征
        concat_features = torch.cat(intermediate_features, dim=-1)  # (B, 2304)
        
        # 投影到2048D
        output = self.feature_projection(concat_features)  # (B, 2048)
...
class TransductiveTrainer:
    ...
    def train(self, num_epochs, episodes_per_epoch, eval_episodes, save_path=None):
        print("\n" + "=" * 70)
        print("  Transductive Fine-Tuning Training")
        print("=" * 70)
...
def main():
    ...
    parser.add_argument("--save_path", default="ssamba_transductive_best.pth")
    ...
    print("=" * 70)
    print("  SSAMBA Transductive Fine-Tuning for Few-Shot Audio Classification")
    print("=" * 70)
