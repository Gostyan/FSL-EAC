#!/usr/bin/env python3
"""
PANNS Transductive Fine-Tuning for Few-Shot Audio Classification

Baseline: Replace SSAMBA with PANNS Cnn14 as feature extractor
- Support Set: 数据增强 -> PANNS特征提取 -> 原型初始化分类器W -> 计算交叉熵损失
- Query Set: PANNS特征提取 -> 计算熵正则化损失 (鼓励低熵预测)
- 总损失 = Support CE + λ * Query Entropy
- 更新参数: θ (PANNS encoder), W, b

Key Differences from SSAMBA:
- Feature extractor: PANNS Cnn14 (64 mel bins, 2048-dim) vs SSAMBA (128 mel bins, 768-dim)
- Freezing: First 5/6 conv blocks frozen (~83% match with SSAMBA)
- All other aspects identical: data augmentation, transductive training, hyperparameters
"""
...
class PANNSEncoder(nn.Module):
    """PANNS Cnn14 编码器 - 接受预提取的64-bin LogMel频谱图"""
    ...
    def _load_pretrained_model(self, pretrained_path):
        ...
        # 提取需要的组件（跳过spectrogram/logmel提取器）
        self.bn0 = model.bn0
        ...
        print(f"[PANNSEncoder] Embedding dim: {self.embed_dim}, Input: {self.n_mels} mel bins")
    ...
    def _freeze_early_blocks(self, n_blocks):
        ...
        print(f"[PANNSEncoder] Frozen: bn0 + conv_block1-5 + conv_block6.conv1/bn1 + fc1")
        print(f"[PANNSEncoder] Trainable: conv_block6.conv2/bn2 (~35% due to PANNS architecture)")
...
def main():
    ...
    parser.add_argument("--save_path", default="panns_transductive_best.pth")
    ...
    print("=" * 70)
    print("  PANNS Transductive Fine-Tuning (Matched ~42M Parameters)")
    print("=" * 70)
    print(f"\n[Matched Params] PANNS: ~42M trainable (conv2/bn2+fc1) vs SSAMBA: ~44M (freeze 13 layers)")
    print(f"[Baseline] PANNS Cnn14 (64 mel bins, 2048-dim) vs SSAMBA (128 mel bins, 768-dim)")
